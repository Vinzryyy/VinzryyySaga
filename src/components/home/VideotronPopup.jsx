/**
 * VideotronPopup — one-shot campaign modal for the Videotron Project.
 *
 * Identical mechanic to AnnouncementPopup (Photo Frame): shows on first
 * Home visit (~1.5s after mount), dismisses to localStorage. Re-openable
 * via 'videotron:open' custom event from Home card / Navbar entry.
 */

import React, { useEffect, useState } from 'react';
import { VIDEOTRON_CAMPAIGN } from '../../config/siteConfig';

const POPUP_VERSION = 'videotron-2026';
const STORAGE_KEY = `armeniaca-popup-${POPUP_VERSION}`;
const SHOW_DELAY_MS = 1800; // slightly after photo-frame popup (1500)

const VideotronPopup = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // localStorage blocked — session-only dismiss is fine.
    }
    const id = window.setTimeout(() => setVisible(true), SHOW_DELAY_MS);
    return () => window.clearTimeout(id);
  }, []);

  // Manual re-open via custom event.
  useEffect(() => {
    const onOpen = () => setVisible(true);
    window.addEventListener('videotron:open', onOpen);
    return () => window.removeEventListener('videotron:open', onOpen);
  }, []);
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="videotron-popup-title"
      ...
      <div className="relative w-full max-w-md md:max-w-lg max-h-[92vh] overflow-y-auto rounded-3xl bg-[color:var(--retro-cream)] shadow-2xl ring-1 ring-[color:var(--retro-gold)]/40 animate-popup-card">
        ...
        <div className="p-6 sm:p-8">
          ...
          <p className="text-sm md:text-base text-[color:var(--color-text-secondary)] leading-relaxed mb-5">
            ...
            Ada 2 jenis videotron yang tayang bolak-balik, jangan lupa cek kedua sisi!
          </p>

          {/* Date strip */}
          <div className="mb-4 flex items-center gap-3 px-4 py-3 rounded-xl bg-[color:var(--retro-burgundy)]/8 border border-[color:var(--retro-burgundy)]/15">
          ...
          <ul className="space-y-1.5 mb-6 text-sm text-[color:var(--retro-text-secondary)]">
            <li className="flex items-start gap-2">
              ...
              <span>Pillar MRT Blok M — 3 pilar di depan Taman Literasi</span>
            </li>
            <li className="flex items-start gap-2">
              <i className="ri-checkbox-circle-line text-[color:var(--retro-burgundy)] mt-0.5 flex-shrink-0" />
              <span>Jl. Panglima Polim, Blok M, Jakarta</span>
            </li>
            <li className="flex items-start gap-2">
              <i className="ri-checkbox-circle-line text-[color:var(--retro-burgundy)] mt-0.5 flex-shrink-0" />
              <span>6 layar (double-sided) — cek kedua sisi!</span>
            </li>
          </ul>
