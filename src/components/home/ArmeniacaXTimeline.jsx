/**
 * ArmeniacaXTimeline — embed @armeniaca15's X timeline using the
 * official widgets.js script. Lazy-mounts via IntersectionObserver
 * so the third-party script and network calls don't block the rest
 * of Home from settling first.
 *
 * Falls back to a "Buka di X" link if the script fails to load
 * (script blocker, X outage, etc.) so the slot is never empty.
 */
...
const ArmeniacaXTimeline = () => {
  ...
  const username = handle.split('/').pop() || 'armeniaca15';
...
export default ArmeniacaXTimeline;
