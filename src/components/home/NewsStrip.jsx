/**
 * NewsStrip — official JKT48 news headlines from /api/v1/news.
 *
 * Reads /data/jkt48-news.json (refreshed every 6h with the schedule
 * scrape) and renders a horizontal-scroll strip of news cards.
...
const NewsCard = ({ item }) => {
  ...
  return (
    <a
      ...
      className="group flex-shrink-0 w-[300px] sm:w-[360px] rounded-2xl overflow-hidden bg-white border border-[color:var(--retro-brown-dark)]/10 hover:border-[color:var(--retro-burgundy)]/40 hover:-translate-y-0.5 hover:shadow-lg transition-all snap-start"
    >
      ...
      <div className="p-4 flex flex-col gap-2">
        ...
        <p className="font-bold text-sm text-[color:var(--retro-text-primary)] leading-snug line-clamp-3 min-h-[3.6em]">
          {item.title}
        </p>
        <span className="mt-1 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.25em] text-[color:var(--retro-burgundy)] group-hover:translate-x-0.5 transition-transform">
          Baca selengkapnya
...
const NewsStrip = () => {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch('/data/jkt48-news.json', { cache: 'no-cache' })
  ...
  return (
    <section
      aria-label="Berita JKT48 terbaru"
