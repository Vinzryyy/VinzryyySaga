...
const SchedulePage = () => {
  ...
  return (
    <main className="relative bg-[color:var(--retro-bg-primary)] min-h-screen">
      ...
      {/* Sedang Dijual — manual merch list (t-shirts, photobook bonuses,
          anything not surfaced by the API). Renders nothing if the
          products array is empty so the page collapses cleanly. */}
      {SITE_CONFIG.sedangDijual && SITE_CONFIG.sedangDijual.products?.length > 0 && (
        <section
          id="sedang-dijual"
          className="px-5 sm:px-6 md:px-12 lg:px-20 py-10 md:py-14"
        >
          <div className="max-w-7xl mx-auto">
            <div className="mb-6 md:mb-8 max-w-3xl">
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[color:var(--retro-burgundy)] mb-3 inline-flex items-center gap-2">
                ...
                {SITE_CONFIG.sedangDijual.eyebrow}
              </p>
              <h2 className="font-header text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter leading-[0.95] text-[color:var(--retro-text-primary)] mb-3">
                {SITE_CONFIG.sedangDijual.title}
                <span className="text-[color:var(--retro-burgundy)]"> {SITE_CONFIG.sedangDijual.titleAccent}</span>
              </h2>
              <p className="text-sm md:text-base text-[color:var(--color-text-secondary)] leading-relaxed">
                {SITE_CONFIG.sedangDijual.lead}
              </p>
            </div>

            {/* Stack of merch cards. Single-column on mobile, two-up on
                lg+ so each card still has room for the photo + spec
                column without becoming cramped. */}
            <div className="grid lg:grid-cols-2 gap-6 md:gap-8">
              {SITE_CONFIG.sedangDijual.products.map((product, idx) => (
                <article
                  key={`${product.name}-${idx}`}
                  className="relative overflow-hidden rounded-3xl border border-[color:var(--retro-burgundy)]/15 bg-[color:var(--retro-cream)]"
                >
                  <div
                    aria-hidden="true"
                  ...
                  <div className="relative grid sm:grid-cols-12 gap-5 sm:gap-6 p-5 sm:p-6 md:p-7 items-start">
                    <div className="sm:col-span-5 relative">
                      <div className="relative rounded-2xl overflow-hidden shadow-xl shadow-[color:var(--retro-brown-dark)]/15 bg-[color:var(--retro-brown-dark)]/5 aspect-[4/5]">
                        <img
                          src={product.image}
                    ...
                    <div className="sm:col-span-7">
                      ...
                      <h3 className="font-header text-base md:text-lg font-black text-[color:var(--retro-text-primary)] leading-snug mb-3">
                        {product.name}
                      </h3>
                      ...
                      <div className="flex items-baseline gap-2 flex-wrap mb-3">
                        ...
                        {product.priceNote && (
                          <span className="text-[10px] md:text-[11px] font-bold text-[color:var(--color-text-muted)]">
                            {product.priceNote}
                          </span>
                        )}
                      </div>
                      ...
                      {product.sizes && product.sizes.length > 0 && (
                        <div className="mb-3">
                          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[color:var(--color-text-muted)] mb-1.5">
                            Ukuran tersedia
                          </p>
                          <div className="flex flex-wrap gap-1.5">
                            {product.sizes.map((size) => (
                              <span
                                key={size}
                                className="inline-flex items-center justify-center min-w-[34px] h-6 px-2 rounded-md bg-[color:var(--retro-burgundy)]/10 text-[color:var(--retro-burgundy)] text-[10px] font-black tracking-wide"
                              >
                                {size}
                              </span>
                            ))}
                      ...
                      <ul className="space-y-1.5 text-xs text-[color:var(--color-text-secondary)] mb-4">
                        {product.preorderUntil && (
                          <li className="flex items-start gap-2">
                            <i className="ri-time-line text-[color:var(--retro-burgundy)] mt-0.5" />
                            <span>
                              Pre-order s/d <strong className="text-[color:var(--retro-text-primary)]">{product.preorderUntil}</strong>
                            </span>
                          </li>
                        )}
                        {product.bonus && (
                      ...
                      {product.link ? (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="group inline-flex items-center gap-2.5 px-5 py-3 rounded-full bg-[color:var(--retro-burgundy)] text-[color:var(--retro-cream)] font-bold text-[11px] md:text-xs uppercase tracking-widest shadow-lg shadow-[color:var(--retro-burgundy)]/30 hover:shadow-xl hover:-translate-y-0.5 transition-all"
                        >
                        ...
                        <span
                          role="status"
                          aria-disabled="true"
                          className="inline-flex items-center gap-2.5 px-5 py-3 rounded-full bg-[color:var(--retro-brown-dark)]/10 text-[color:var(--color-text-muted)] font-bold text-[11px] md:text-xs uppercase tracking-widest cursor-not-allowed select-none"
                        >
